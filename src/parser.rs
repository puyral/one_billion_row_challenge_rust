...
fn find_next(data: &[u8]) -> Option<(usize, usize)> {
    match sawr_search(data) {
        Some(idsc) => Some((idsc, find_temperature(&data[idsc + 1..]) + idsc + 1)),
        None if data.len() >= NUMER_SKIPPED => slow_search(data),
        _ => None,
    }
}

fn sawr_search(data: &[u8]) -> Option<usize> {
...
fn slow_search(data: &[u8]) -> Option<(usize, usize)> {
    let idsc = memchr::memchr(b';', &data[NUMER_SKIPPED - 1..])? + NUMER_SKIPPED - 1;
    let idnl = find_temperature(&data[idsc + 1..]) + idsc + 1;
    Some((idsc, idnl))
}
...
fn find_temperature(data: &[u8]) -> usize {
    if data.len() < 8 {
        // SIMD
